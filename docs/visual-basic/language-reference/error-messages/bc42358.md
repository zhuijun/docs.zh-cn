...
---
# <a name="because-this-call-is-not-awaited-execution-of-the-current-method-continues-before-the-call-is-completed"></a>在调用完成之前，会继续执行当前方法，原因是此调用不处于等待状态

在调用完成之前，会继续执行当前方法，原因是此调用不处于等待状态。 请考虑将 `Await` 运算符应用于调用的结果。
...
2. 在 Visual Studio 代码编辑器中，选择 **“MainWindow.xaml”** 选项卡。
     ...
     如果该选项卡不可见，请在**解决方案资源管理器**中打开 mainwindow.xaml 的快捷菜单，然后选择 "**查看代码**"。
