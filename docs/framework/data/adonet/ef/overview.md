...
## <a name="give-life-to-models"></a>为模型指定生存期
 ...
 实体框架通过使开发人员能够查询域模型中的实体和关系， (在实体框架) 中称为 *概念* 模型，同时依赖于实体框架将这些操作转换为特定于数据源的命令，从而为模型提供了模型。 这使应用程序不再对特定数据源具有硬编码的依赖性。
...
[入门](getting-started.md) -提供有关如何使用快速 [入门](/previous-versions/dotnet/netframework-4.0/bb399182(v=vs.100))快速启动和运行的信息，其中演示了如何创建简单的实体框架应用程序。
