...
<span data-ttu-id="e7365-158">在 .NET Native 中：</span><span class="sxs-lookup"><span data-stu-id="e7365-158">In .NET Native:</span></span>
...
- <span data-ttu-id="e7365-187">不支持大于 1 mb 的值类型。</span><span class="sxs-lookup"><span data-stu-id="e7365-187">Value types larger than 1 megabyte aren't supported.</span></span>
...
- <span data-ttu-id="e7365-204">指针阵列不受支持。</span><span class="sxs-lookup"><span data-stu-id="e7365-204">Arrays of pointers aren't supported.</span></span>
...
<span data-ttu-id="e7365-234">**自动重定向**</span><span class="sxs-lookup"><span data-stu-id="e7365-234">**Automatic redirection**</span></span>
...
<span data-ttu-id="e7365-256">在 .NET Native 中：</span><span class="sxs-lookup"><span data-stu-id="e7365-256">In .NET Native:</span></span>
...
- <xref:System.Runtime.InteropServices.UnmanagedType.CustomMarshaler?displayProperty=fullName>
 ...
 <span data-ttu-id="e7365-279">很少使用的封送 Api：</span><span class="sxs-lookup"><span data-stu-id="e7365-279">Rarely used marshaling APIs:</span></span>
...
- <span data-ttu-id="e7365-290">所有 WinRT 构造</span><span class="sxs-lookup"><span data-stu-id="e7365-290">All WinRT constructs</span></span>
...
- <xref:System.ComponentModel.DataAnnotations.Schema.DatabaseGeneratedOption?displayProperty=nameWithType>
 ...
 <span data-ttu-id="e7365-307">**Visual Basic**</span><span class="sxs-lookup"><span data-stu-id="e7365-307">**Visual Basic**</span></span>
...
### <a name="differences-in-serializers"></a><span data-ttu-id="e7365-317">序列化程序中的差异</span><span class="sxs-lookup"><span data-stu-id="e7365-317">Differences in serializers</span></span>
...
- <span data-ttu-id="e7365-328">以下序列化类型的构造函数无法保证按照预期工作：</span><span class="sxs-lookup"><span data-stu-id="e7365-328">The following constructors of the serialization types aren't guaranteed to work as expected:</span></span>
...
- <span data-ttu-id="e7365-329"><xref:System.Xml.Serialization.XmlSerializer> 无法为具有以下任意特性的方法的类型生成代码：</span><span class="sxs-lookup"><span data-stu-id="e7365-329"><xref:System.Xml.Serialization.XmlSerializer> fails to generate code for a type that has methods attributed with any of the following attributes:</span></span>
...
- <span data-ttu-id="e7365-330"><xref:System.Xml.Serialization.XmlSerializer> 不兼容 <xref:System.Xml.Serialization.IXmlSerializable> 自定义序列化接口。</span><span class="sxs-lookup"><span data-stu-id="e7365-330"><xref:System.Xml.Serialization.XmlSerializer> doesn't honor the <xref:System.Xml.Serialization.IXmlSerializable> custom serialization interface.</span></span> <span data-ttu-id="e7365-331">如果你的类实施此接口， <xref:System.Xml.Serialization.XmlSerializer> 会考虑普通旧 CLR 对象 (POCO) 类型并仅对其公共属性进行序列化。</span><span class="sxs-lookup"><span data-stu-id="e7365-331">If you have a class that implements this interface, <xref:System.Xml.Serialization.XmlSerializer> considers the type a plain old CLR object (POCO) type and serializes only its public properties.</span></span>

- <span data-ttu-id="e7365-332">序列化纯 <xref:System.Exception> 对象并不适合 <xref:System.Runtime.Serialization.DataContractSerializer> 和 <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> 。</span><span class="sxs-lookup"><span data-stu-id="e7365-332">Serializing a plain <xref:System.Exception> object doesn't work well with <xref:System.Runtime.Serialization.DataContractSerializer> and <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer>.</span></span>

<a name="VS"></a>

## <a name="visual-studio-differences"></a><span data-ttu-id="e7365-333">Visual Studio 差异</span><span class="sxs-lookup"><span data-stu-id="e7365-333">Visual Studio differences</span></span>

<span data-ttu-id="e7365-334">**异常和调试**</span><span class="sxs-lookup"><span data-stu-id="e7365-334">**Exceptions and debugging**</span></span>

<span data-ttu-id="e7365-335">在调试器中运行使用 .NET Native 编译的应用时，会为以下异常类型启用第一次异常：</span><span class="sxs-lookup"><span data-stu-id="e7365-335">When you're running apps compiled by using .NET Native in the debugger, first-chance exceptions are enabled for the following exception types:</span></span>

- <xref:System.MemberAccessException>

- <xref:System.TypeAccessException>

<span data-ttu-id="e7365-336">**构建应用程序**</span><span class="sxs-lookup"><span data-stu-id="e7365-336">**Building apps**</span></span>

<span data-ttu-id="e7365-337">使用 Visual Studio 默认使用的 x86 构建工具。</span><span class="sxs-lookup"><span data-stu-id="e7365-337">Use the x86 build tools that are used by default by Visual Studio.</span></span> <span data-ttu-id="e7365-338">我们不推荐使用 AMD64 MSBuild 工具（位置路径为 C:\Program Files (x86)\MSBuild\12.0\bin\amd64），因为这些工具会导致构建问题。</span><span class="sxs-lookup"><span data-stu-id="e7365-338">We don't recommend using the AMD64 MSBuild tools, which are found in C:\Program Files (x86)\MSBuild\12.0\bin\amd64; these may create build problems.</span></span>

<span data-ttu-id="e7365-339">**探查器**</span><span class="sxs-lookup"><span data-stu-id="e7365-339">**Profilers**</span></span>

- <span data-ttu-id="e7365-340">Visual Studio CPU 探查器和 XAML 内存探查器不会正确显示 Just-My-Code。</span><span class="sxs-lookup"><span data-stu-id="e7365-340">The Visual Studio CPU Profiler and XAML Memory Profiler don't display Just-My-Code correctly.</span></span>

- <span data-ttu-id="e7365-341">XAML 内存探查器不会精确显示托管的堆数据。</span><span class="sxs-lookup"><span data-stu-id="e7365-341">The XAML Memory Profiler doesn't accurately display managed heap data.</span></span>

- <span data-ttu-id="e7365-342">CPU 探查器不会正确区分模块和显示前缀的功能名。</span><span class="sxs-lookup"><span data-stu-id="e7365-342">The CPU Profiler doesn't correctly identify modules, and displays prefixed function names.</span></span>

<span data-ttu-id="e7365-343">**单元测试库项目**</span><span class="sxs-lookup"><span data-stu-id="e7365-343">**Unit Test Library projects**</span></span>

<span data-ttu-id="e7365-344">不支持在 Windows 应用商店应用项目的单元测试库上启用 .NET Native，并导致项目未能生成。</span><span class="sxs-lookup"><span data-stu-id="e7365-344">Enabling .NET Native on a Unit Test Library for a Windows Store apps project isn't supported and causes the project to fail to build.</span></span>

## <a name="see-also"></a><span data-ttu-id="e7365-345">另请参阅</span><span class="sxs-lookup"><span data-stu-id="e7365-345">See also</span></span>

- [<span data-ttu-id="e7365-346">入门</span><span class="sxs-lookup"><span data-stu-id="e7365-346">Getting Started</span></span>](getting-started-with-net-native.md)
- [<span data-ttu-id="e7365-347">运行时指令 (rd.xml) 配置文件引用</span><span class="sxs-lookup"><span data-stu-id="e7365-347">Runtime Directives (rd.xml) Configuration File Reference</span></span>](runtime-directives-rd-xml-configuration-file-reference.md)
- [<span data-ttu-id="e7365-348">适用于 Windows 应用商店应用的 .NET 概述</span><span class="sxs-lookup"><span data-stu-id="e7365-348">.NET For Windows Store apps overview</span></span>](https://docs.microsoft.com/previous-versions/windows/apps/br230302%28v=vs.140%29)
- [<span data-ttu-id="e7365-349">.NET Framework 对 Windows 应用商店应用程序和 Windows 运行时的支持情况</span><span class="sxs-lookup"><span data-stu-id="e7365-349">.NET Framework Support for Windows Store Apps and Windows Runtime</span></span>](../../standard/cross-platform/support-for-windows-store-apps-and-windows-runtime.md)
