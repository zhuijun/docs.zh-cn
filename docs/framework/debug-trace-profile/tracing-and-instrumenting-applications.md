...
- 性能计数器 - 可用于跟踪应用程序性能的组件****。 有关详细信息，请参阅[性能计数器](performance-counters.md)。  
...
|`WriteLineIf`|如果满足指定为语句中的参数的条件，则为指定的文本和一个回车符 `WriteLineIf` 。|  
 ...
 `Write`和 `WriteLine` 方法始终写入指定的文本。 `Assert`、 `WriteIf` 和 `WriteLineIf` 需要一个布尔参数，用于控制它们是否写入指定的文本; 只有在表达式为**true** （对于 `WriteIf` 和 `WriteLineIf` ）或**false** （对于）时，它们才会写入指定的文本 `Assert` 。 `Fail`方法始终写入指定的文本。 有关详细信息，请参阅[如何：向应用程序代码添加跟踪语句](how-to-add-trace-statements-to-application-code.md)和 .NET Framework 参考。  
