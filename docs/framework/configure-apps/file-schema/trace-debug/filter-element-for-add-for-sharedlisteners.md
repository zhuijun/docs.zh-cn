...
|属性|说明|  
|---------------|-----------------|  
|type|必需的特性。<br /><br /> 指定筛选器的类型。 您只能使用该类型的完整名称（采用 <xref:System.Type.FullName%2A?displayProperty=nameWithType> 属性格式），也可以使用包含程序集信息（采用属性格式）的完全限定的类型名称 <xref:System.Type.AssemblyQualifiedName%2A?displayProperty=nameWithType> 。 有关创建完全限定类型名称的信息，请参阅[指定完全限定的类型](../../../reflection-and-codedom/specifying-fully-qualified-type-names.md)名称。|  
