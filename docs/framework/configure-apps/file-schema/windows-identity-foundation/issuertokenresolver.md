...
## <a name="example"></a>示例  
 下面的 XML 显示基于从派生的自定义类的颁发者令牌解析程序的配置 <xref:System.IdentityModel.Tokens.IssuerTokenResolver> 。 令牌解析程序维护从为类定义的自定义配置元素（）初始化的访问群体-密钥对的字典 `<AddAudienceKeyPair>` 。 类将重写 <xref:System.IdentityModel.Selectors.SecurityTokenResolver.LoadCustomConfiguration%2A> 方法来处理此元素。 下面的示例显示了替代：但对于简洁起见，它调用的方法不会显示。 有关完整的示例，请参阅 `CustomToken` 示例。  
