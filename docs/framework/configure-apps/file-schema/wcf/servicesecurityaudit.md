...
|messageAuthenticationAuditLevel|指定所记录的消息身份验证审核事件的类型。 有效值包括以下值：<br /><br /> -None：不生成审核事件。<br />-Success：只记录成功的安全性（包括消息签名验证、密码和令牌验证的完全验证）事件。<br />-失败：只记录失败事件。<br />-SuccessOrFailure：成功和失败事件都被记录。<br /><br /> 默认值为 None。 有关详细信息，请参阅 <xref:System.ServiceModel.AuditLevel>。|  
