...
## <a name="remarks"></a>注解  
 如果的值 `reason` 为 COR_PRF_TRANSITION_CALL，则函数 ID 为非托管函数的 ID，该函数从未使用实时编译器进行编译。 非托管函数具有与之关联的基本信息，如名称和某些元数据。 如果使用隐式平台调用（PInvoke）调用了非托管函数，则运行时无法确定调用的目标，的值 `functionId` 将为 null。 有关隐式 PInvoke 的详细信息，请参阅[使用 c + + 互操作（隐式 pinvoke）](/cpp/dotnet/using-cpp-interop-implicit-pinvoke)。  
