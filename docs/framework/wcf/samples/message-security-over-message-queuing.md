...
5. 编辑服务的 setup.exe，以反映新的证书名称（在的属性中），该名称与 `findValue` [\<serviceCertificate>](../../configure-apps/file-schema/wcf/servicecertificate-of-servicecredentials.md) 计算机的完全限定域名相同。  
...
- 创建服务器证书。
    ...
    ```
     ...
     %SERVER_NAME% 变量指定服务器名称。 该证书存储在 LocalMachine 存储区中。 如果使用服务参数（如）运行 setup 批处理文件， `setup.bat service` 则% SERVER_NAME% 包含计算机的完全限定域名。否则，默认为 localhost
