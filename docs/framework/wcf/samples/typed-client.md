...
ms.locfileid: "84591171"
---
# <a name="typed-client"></a>类型化客户端
此示例演示如何从工作信息[元数据实用工具（svcutil.exe）](../servicemodel-metadata-utility-tool-svcutil-exe.md)生成的类型化客户端获取信息。 此示例基于实现计算器服务的[入门](getting-started-sample.md)。 在此示例中，客户端是一个控制台应用程序 (.exe)，服务是由 Internet 信息服务 (IIS) 承载的。  
